...
class ObjectDetectionSystem:
    def __init__(self):
        ...
        try:
            ...
            self.classes = ['face']
            self.colors = [(0, 255, 0)]  # Green for faces
...
def main():
    ...
    <div class="main-header">
        ...
        <p>Upload images to detect faces!</p>
    </div>
    """, unsafe_allow_html=True)
        ...
        if st.button("🔍 Detect Objects", type="primary"):
            with st.spinner("Processing..."):
                ...
                if result_image is not None:
                    ...
                    # Statistics
                    st.success(f"Found {len(detections)} faces!")
                    
                    if detections:
                        for i, detection in enumerate(detections):
                            st.write(f"Face {i+1}: confidence {detection['confidence']:.2f}")
                else:
